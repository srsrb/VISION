...
void build_graph(Graph<int,int,int>& G,
    ...
    int w = I2M.width();
    double kp = 1 + (nd - 1) * 4 * lambda;
    G.add_node(nd * nx * ny);


    float weight; float weight2; float rho; int node; int next_node;
    double zncct;
    for (int x = 0; x < nx; x++){
        for (int y = 0; y < ny; y++){
            for (int d = 0; d < nd; d++) {
                node = x+nx*(y+ny*d);
                next_node = x+nx*(y+ny*(d+1));

                if (x < (w - 2 * win - dmin - d) / zoom){
                    zncct = zncc(I1, I1M, I2, I2M, win + zoom * x, win + zoom * y, win + zoom * x + dmin + d, win + zoom * y);
                    if (zncct > 0)
                        rho = sqrt(1 - zncct);
                    else
                        rho = 1;
                ...
                if (d == 0){ // first ?
                    G.add_tweights(node, weight + kp, 0);
                    G.add_edge(node, next_node, weight + kp, 0);
                }
                else if (d == nd-1){ // last ?
                    if (x < (w - 2 * win - dmax) / zoom){
                        zncct = zncc(I1, I1M, I2, I2M, win + zoom * x, win + zoom * y, win + zoom * x + dmax, win + zoom * y);
                        if (zncct > 0)
                            rho = sqrt(1 - zncct);
                        else
                            rho = 1;
                    ...
                    G.add_tweights(node, 0, weight2 + kp);
                }
                else
                    G.add_edge(node, next_node, weight + kp, 0);
                ...
                if (x > 0)
                    G.add_edge(node, (x-1)+nx*(y+ny*d), lambda, lambda);

                if (y > 0)
                    G.add_edge(node, x+nx*((y-1)+ny*d), lambda, lambda);
            }
        }
    }
}
...
doubleImage decode_graph(Graph<int,int,int>& G, int nx, int ny, int nd, int w2){
    ...
    for (int x = 0; x < nx; x++)
        for (int y = 0; y < ny; y++)
        {
            ...
            {
                d++;
            } while (G.what_segment(x+nx*(y+ny*d)) == Graph<int, int, int>::SOURCE);
